...
Pipeline <- R6::R6Class("Pipeline", # nolint
  public = list(
    ...
    discard_steps = function(pattern,
                             recursive = FALSE,
                             fixed = TRUE,
                             ...) {
      steps2remove <- grep(
    ...
    get_depends_down = function(step,
                                recursive = TRUE) {
    ...
    #' p
    insert_after = function(afterStep,
                            step,
                            ...) {
      private$.verify_step_does_not_exist(step)
    ...
    #' p
    insert_before = function(beforeStep,
                             step,
                             ...) {
      private$.verify_step_does_not_exist(step)
    ...
    #'
    #' p$run()$collect_out()
    set_data_split = function(dataList,
                              toStep = character(),
                              groupBySplit = TRUE,
                              sep = ".") {
      if (length(toStep) == 0) {
    ...
    set_params_at_step = function(step,
                                  params) {
      stopifnot(
        is_string(step),
        is.list(params)
  ...
  private = list(
    ...
    .derive_dependencies = function(params,
                                    step,
    ...
    .extract_dependent_out = function(depends,
                                      out) {
      stopifnot(
    ...
    .get_downstream_depends = function(step,
                                       depends,
                                       recursive = TRUE) {
      stopifnot(
        is_string(step),
    ...
    .get_upstream_depends = function(step,
                                     depends,
                                     recursive = TRUE) {
      stopifnot(
        is_string(step),
    ...
    .prepare_and_verify_params = function(fun,
                                          funcName,
                                          params = list()) {
      stopifnot(
        is.function(fun),
    ...
    .relative_dependency_to_index = function(relative_dep,
                                             dependencyName,
                                             startIndex,
    ...
    .verify_dependency = function(dep,
                                  step, # required for error message
                                  toStep = private$.get_last_step()) {
      stopifnot(
        is_string(dep),
    ...
    .verify_fun_params = function(fun,
                                  funcName,
