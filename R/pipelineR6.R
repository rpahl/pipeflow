...
Pipeline <- R6::R6Class(
    ...
    public = list(
        ...
        initialize = function(name, data = NULL, logger = NULL) {
            topCall <- paste(deparse(sys.calls()[[1L]]), collapse = "")
            calledDirectly <- grepl("Pipeline\\$new\\s*\\(", topCall)
            if (
                calledDirectly &&
                    !isTRUE(getOption(
                        "pipeflow.suppress_pipeline_new_deprecation"
                    )) &&
                    !identical(
            ...
            if (is.function(logger)) {
                ...
                if (!setequal(names(formals(logger)), expectedFormals)) {
                    stop_no_call(
                        ...
                        "function(",
                        paste(expectedFormals, collapse = ", "),
                        ")"
                    )
                }
                private$.lg <- logger
        ...
        collect_out = function(groupBy = "group", all = FALSE) {
            ...
            stopifnot(
                ...
                "groupBy column does not exist" = groupBy %in%
                    colnames(self$pipeline),
                "groupBy column must be character" = is.character(self$pipeline[[
                    groupBy
                ]])
            )
            # nolint end
        ...
        get_depends_down = function(step, recursive = TRUE) {
        ...
        get_depends_up = function(step, recursive = TRUE) {
            private$.verify_step_exists(step)
        ...
        #' }
        get_graph = function(groups = NULL) {
        ...
        #' p
        insert_after = function(afterStep, step, ...) {
            private$.verify_step_does_not_exist(step)
        ...
        #' p
        insert_before = function(beforeStep, step, ...) {
            private$.verify_step_does_not_exist(step)
        ...
        #' p
        remove_step = function(step, recursive = FALSE) {
            private$.verify_step_exists(step)
        ...
        set_params_at_step = function(step, params) {
            stopifnot(
                is_string(step),
                is.list(params)
            )
    ...
    private = list(
        ...
        .create_edge_table = function(groups = NULL) {
            deps <- self$get_depends()
        ...
        .create_node_table = function(groups = NULL) {
            pip <- self$pipeline
        ...
        .extract_dependent_out = function(depends, out) {
            stopifnot(
        ...
        .extract_depends_from_param_list = function(params) {
            if (length(params) == 0) {
                return(character())
            }
        ...
        .get_downstream_depends = function(step, depends, recursive = TRUE) {
            stopifnot(
                is_string(step),
                is.character(depends) || is.list(depends),
                is.logical(recursive)
        ...
        .get_upstream_depends = function(step, depends, recursive = TRUE) {
            stopifnot(
                is_string(step),
                is.character(depends) || is.list(depends),
                is.logical(recursive)
        ...
        .prepare_and_verify_params = function(fun, funcName, params = list()) {
            stopifnot(
                is.function(fun),
                is_string(funcName),
                is.list(params)
        ...
        .set_at_step = function(step, field, value) {
