...
```{r, echo = FALSE}
library(visNetwork)
do.call(visNetwork, args = c(pip_get_graph(pip), list(height = 250, width = 500))) |>
...
```{r}
pip_replace(pip, "data2", function(data = ~standardize) data)
...
```{r, echo = FALSE}
library(visNetwork)
do.call(visNetwork, args = c(pip_get_graph(pip), list(height = 100, width = 700))) |>
    visHierarchicalLayout(direction = "LR")
```
...
```{r}
pip_replace(pip, "data2", function(data = ~-1) data)
...
Another way to re-use steps from other pipelines is by cherry-picking, for
example:
...
pip
```
In this instance, however, the cherry-pick approach was not all that useful,
because in contrast to the `pip_bind` command, which renamed `data` to `data2`,
the `fit` and `plot` step will still refer to the initial `data` step.

```{r, echo = FALSE}
library(visNetwork)
do.call(visNetwork, args = c(pip_get_graph(pip), list(height = 250, width = 700))) |>
    visHierarchicalLayout(direction = "LR", sortMethod = "directed")
```

In other scenarios, however, this can be exactly what is desired.
...
interested in (e.g. the `plot` output the above example).
To see how {pipeflow} allows to conveniently tag, collect and possibly group 
the output of those final results, 
see the next vignette [Collecting output](v04-collect-output.html).
