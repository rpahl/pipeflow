...
need to call `pip_restart(.self)`, so we adapt the `f2` function as
follows:
...
the pipeline was modified and, since the step called
`pip_restart(.self)`, the pipeline is also restarted automatically
