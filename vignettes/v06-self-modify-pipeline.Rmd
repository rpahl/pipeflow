...
For this, the pipeline by default provides a self-reference in the
form of the `.self` object that is automatically made available in
any step function.
With this in mind, we can update the `residual_shapiro_p_value`
step to change the `pointColor` parameter on the fly.
...
runtime are not limited to changing parameters but can also be used
to modify the very own pipeline structure.

### Changing pipeline structure at runtime

Subsequently, the pipeline steps will be comprised only of very basic functions
in order to keep matters simple. The focus here should be on the pipeline
structure and how it is modified.
...
For our last example, we get even more hacky and instead of just replacing,
...
actually the new step `f2b` was run^[
    Compare the state of `f2b`, which is set to `done`.
]
...
and temporarily showing a wrong log is not ideal. Ideally, the
pipeline run would be aborted right after all changes were
done in `f2` and the pipeline re-run automatically from the beginning.
...
need to return the pipeline object, so we adapt the `f2` function as
follows:
...
Second, you need to run the pipeline with `recursive = TRUE`.
...
Of course, this was just a silly example to show some possibilities,
