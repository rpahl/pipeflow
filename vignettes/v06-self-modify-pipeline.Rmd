---
title: "Advanced: Recursive self-modification at runtime"
output:
...
vignette: >
  %\VignetteIndexEntry{Advanced: Recursive self-modification at runtime}
...
if (pip[["residual_shapiro_p_value", "out"]] < 0.05) {
    pip |> pip_set_params(list(pointColor = "red")) |> pip_run()
}
...
```{r}
pip |> pip_set_params(list(xInit = 15)) |> pip_run()
...
```{r}
pip |> pip_set_params(list(xInit = 11)) |> pip_run()
...
```{r}
pip |> pip_set_params(list(xInit = 11)) |> pip_run(recursive = TRUE)
```
