...
with the name of the second pipeline (lines 4-6). For example, the first step of the
second pipeline has been renamed from `data` to `data.modeling` (line 4).
This is always done when appending a pipeline to avoid any step name clashes in case
the two pipelines contain steps with identical names.

However, as can be also seen from the graphical representation of the pipeline,
...
`standardize` step as data parameter input in the `data.modeling` step.
...
step has been replaced by the output of the last step of the first pipeline using
the relative indexing mechanism.
