...
We define a pipeline that starts with base R's airquality dataset, adds a new
column (step "data_prep"), fits a linear model (step "model_fit") and finally
plots the model against the data (step "model_plot"). We then show how to
run the pipeline, how to inspect the output of a specific step, how to
change the parameters of the pipeline and how to rerun the pipeline with the
updated parameters.
...
step as can be seen in column `deps`, which will list all dependencies of
a step.
...
parameter as well as a parameter `title`, which is used as the title
of the plot.
...
Note that in the last line for the `model_plot` step, we can see that the
`model_fit` and `data_prep` steps are listed as dependencies.
...
Any parameter of the pipeline that is *not* bound to a specific step can be
...
`model_fit` and `model_plot` steps. We can do this simply as follows:
