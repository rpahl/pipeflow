...
describe("add",
{
    ...
    {
        pip <- pipe_new("pipe")$add("f1", fun = \(x = 1) x)
...
describe("get_step_number",
{
    ...
    {
        expect_true(is.function(pipe_new("pipe")$get_step_number))
...
describe("run",
{
    ...
    {
        expect_no_error(pipe_new("pipe1")$run())
    })

    test_that("returns the pipeline object",
    {
        pip <- pipe_new("pipe1")$run()
